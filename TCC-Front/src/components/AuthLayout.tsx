...
const SAMPLE_ENTRIES = [
  { title: "Salário", category: "Salário", color: "#10B981", value: 4200, type: "income" },
  { title: "Supermercado", category: "Alimentação", color: "#F59E0B", value: 386.9, type: "expenses" },
  { title: "Aluguel", category: "Casa", color: "#3B82F6", value: 1450, type: "expenses" },
  { title: "Fatura do cartão", category: "Cartão", color: "#EF4444", value: 872.4, type: "expenses" },
];

const SAMPLE_BALANCE = SAMPLE_ENTRIES.reduce(
  (total, entry) => (entry.type === "income" ? total + entry.value : total - entry.value),
  0
);
  ...
  return (
    <div className="flex min-h-screen w-full bg-ink-50">
      ...
      <div className="relative hidden overflow-hidden bg-ink-950 p-10 text-white lg:flex lg:w-[52%] lg:flex-col lg:justify-between xl:p-14">
        <span aria-hidden className="absolute -right-24 -top-28 h-96 w-96 rounded-full bg-emerald-500/20 blur-3xl" />
        <span aria-hidden className="absolute -bottom-32 -left-20 h-96 w-96 rounded-full bg-ocean-600/20 blur-3xl" />
        <span
          aria-hidden
          className="absolute inset-x-0 top-0 h-1 bg-brand-gradient"
        />

        <Logo size="lg" plate className="relative z-10 animate-fade-up self-start" />
        ...
        <div className="relative z-10 my-12">
          ...
          <p className="mt-4 max-w-md text-[0.95rem] leading-relaxed text-ink-200 animate-fade-up">
            Cada receita e cada despesa em um só lugar — com o saldo recalculado a cada lançamento.
          </p>
          ...
          >
            ...
            <ul className="flex flex-col divide-y divide-white/[0.07]">
              {SAMPLE_ENTRIES.map((entry) => (
                <li key={entry.title} className="flex items-center gap-3 py-2.5">
                  <span
                  ...
                  >
                    {entry.type === "income" ? "+" : "−"} R$ {money(entry.value)}
                  </p>
                </li>
              ))}
            </ul>
            ...
            <div className="mt-4 flex items-center justify-between rounded-xl bg-emerald-500/15 px-3.5 py-3 ring-1 ring-inset ring-emerald-400/20">
              ...
              <p className="tnum font-display text-lg font-bold text-white">R$ {money(SAMPLE_BALANCE)}</p>
            </div>
          </div>
        </div>

        <p className="relative z-10 text-xs text-ink-400">
          Projeto de TCC — Análise e Desenvolvimento de Sistemas
        </p>
      </div>
